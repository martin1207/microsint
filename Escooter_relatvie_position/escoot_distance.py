...
def dist_est(c_x, c_y):
    distance_features, distance_model = joblib.load('/Users/martin.dejaeghere/PhD/Position_Estimation/escooter/distance_predictor_small.joblib')
    ...
    if cur_dist >thredshold_dist-10:
        distance_features, distance_model = joblib.load('/Users/martin.dejaeghere/PhD/Position_Estimation/escooter/distance_predictor_big.joblib')
        image_data =pd.DataFrame([[c_x, c_y]], columns=["x_center", "y2"])
        cur_dist = distance_model.predict(distance_features.transform(image_data))[0][0]
...
def get_ang_dist_escoot(c_x, c_y):
    width = 5376
